  ...
  <div class="modal-dialog bg-danger">
    <div class="modal-content bg-danger">
      <div class="modal-header">
        <h1 class="modal-title fs-5 text-white" id="exampleModalLabel">{{__('Delete User ')}} {{ $userSelected ? $userSelected->name :'' }}</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"  wire:click="closeModals()"></button>
      </div>
      <div class="modal-body text-white">
        {{__('Are you sure to delete the user')}} {{ $userSelected ? $userSelected->name :'' }}?
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" wire:click="closeModals()">{{__('Close')}}</button>
        <button type="button" class="btn btn-primary" wire:click="confirmRemoveUser()">{{__('Sure')}}</button>
      </div>
    </div>
  </div>
</div>